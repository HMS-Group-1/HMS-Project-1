...
const TablePinjam = () => {
    ...
    const pinjamHandler = async (id) => {
        ...
        const tokenRef = res.data.accessToken;
        try {
            const response = await axios.post(`http://localhost:5000/pinjam/${id}`, {
                data: {
        ...
        <div className='w-full'>
            ...
            <div className="my-2 flex mb-4 desktop:mb-8 desktop:w-1/4">
                ...
                <button
                    ...
                    className='px-4 rounded-md bg-unguTua text-white ml-2'>Cari</button>
            </div>
                    ...
                    <tbody>
                        ...
                        {books && books.filter(book => book.judul_buku.toLocaleLowerCase().includes('')).map((book, index) => (
                            <tr className="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <td className="py-4 px-6">
                                    {toTitleCase(book.Kategori_id.kategori_nama)}
                                </td>
                                <td className="py-4 px-6">
                                    {book.stok}
                                </td>
                                <td className="py-4 px-6 hover:cursor-pointer">
                                    <p onClick={() => pinjamHandler(book.id)} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">Pinjam</p>
                                </td>
                            </tr>
                        ))}
