...
const TablePengembalian = () => {
    ...
    const kembaliHandler = async (id) => {
        ...
        try {
            const response = await axios.patch(`http://localhost:5000/kembalikan/${id}`, {
                data: {
    ...
    const getListPinjam = async () => {
        ...
        const tokenRef = res.data.accessToken
        const response = await axios.get('http://localhost:5000/listPinjam', {
            params: {
                userId: context.userId,
                page: search.page,
                limit: 10,
                        ...
                        {listPinjam?.map((book, index) => (
                            <tr className="bg-white border-b dark:bg-gray-900 dark:border-gray-700" key={index}>
                                ...
                                <td className="py-4 px-6">
                                    {toTitleCase(book.tbl_buku.judul_buku)}
                                </td>
                                <td className="py-4 px-6">
                                    {toTitleCase(book.tbl_buku.Kategori_id.kategori_nama)}
                                </td>
                                <td className="py-4 px-6">
                                    {book.tanggalPinjam}
                                </td>
                                <td className="py-4 px-6">
                                    {toTitleCase(book.tbl_buku.Rak_id.lokasi_rak)}
                                </td>
                                <td className="py-4 px-6">
                                    {book.isPinjam === true ?
                                ...
                                {book.isPinjam === true ?
                                    <td className="py-4 px-6 hover:cursor-pointer">
                                        <p onClick={() => kembaliHandler(book.buku_id)} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">Kembalikan</p>
                                    </td> :
