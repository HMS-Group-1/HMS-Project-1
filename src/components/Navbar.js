			...
			{!context.isLogin ? (
				...
				<div className="flex gap-2 justify-end w-1/2">
					<div className='w-10 h-10 bg-yellow-300 rounded-full'>
						<p onClick={menuHandler} className='w-full h-full relative hover:cursor-pointer flex justify-center items-center text-lg text-white font-medium'>{getInitials(context.name)}</p>
						{menu && <div className='bg-blue-400 z-20 mt-1 rounded-md w-[180px] py-2 absolute right-0 top-11'>
							<ul className='flex flex-col gap-2'>
								...
								<Link to='/'>
									<li className='text-sm hover:bg-blue-500 py-1 text-white pl-2' onClick={logOutHandler}>Keluar</li>
								</Link>
							</ul>
						</div>}
					</div>
